...
class SessionActivator():
    ...
    def __init__(self):
        ...
        self.soup_login = BeautifulSoup(login_url, 'html.parser')


    def activateSession(self):

        loginUrl = 'https://www.wikifolio.com/dynamic/de/de/login/login/'
        headers = {'useragent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.16; rv:83.0) Gecko/20100101 Firefox/83.0'}

        login_url = self.s.get(url='https://www.wikifolio.com/dynamic/de/de/login/login').content


        #### FRAGILE PARAMETER; MIGHT CHANGE DUE TO AMENDMENTS WITHIN EXTERNAL PROD CODE ####
        request_verification_token = soup_login.select_one('input[name="__RequestVerificationToken"]')['value']
        ...
        connectionTokenContext = self.s.get(connectionTokenUrl, headers=headers).text
